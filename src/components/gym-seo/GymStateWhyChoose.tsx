...
import type { GymStateWhyChooseItem } from '@/types';
import { Button } from '@/components/ui/button';
import Link from 'next/link';
import Image from 'next/image';
import React, { ComponentType } from 'react';
import { TrendingUp, Lightbulb, ShieldCheck, Users, Dumbbell, BarChart2 } from 'lucide-react';
...
interface GymStateWhyChooseProps {
  stateName: string;
  headline: string;
  intro: string;
  points: GymStateWhyChooseItem[];
...
export function GymStateWhyChoose({ headline, intro, points, whyChooseParagraph1, whyChooseParagraph2, stateName, videoUrl, videoThumbnailUrl, videoThumbnailAlt }: GymStateWhyChooseProps) {
        ...
        <div className="flex flex-col md:flex-row items-start md:space-x-8">
          <div className="md:w-1/2 mb-6 md:mb-0 flex flex-col justify-between">
            ...
            <div className="text-center md:text-left">
              <p className="text-primary mb-1 text-xl font-bold">Meet Our Fitness SEO Experts</p>
              <p className="text-muted-foreground mb-6 text-base">See how our team can help your {stateName} gym or studio reach its potential!</p>
          ...
          <div className="md:w-1/2 relative group cursor-pointer" onClick={() => setIsModalOpen(true)}>
           ...
           <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
              <iframe className="w-full aspect-video" src={`${videoUrl}?rel=0&showinfo=0&controls=0&autoplay=1`} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title={`TRAC Gym & Fitness SEO for ${stateName}`}></iframe>
            </Modal>
        </div>
