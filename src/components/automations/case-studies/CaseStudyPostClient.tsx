...
interface CaseStudyPostClientProps {
  initialCaseStudyData: any;
  initialRelatedCaseStudies: any[];
}
...
export default function CaseStudyPostClient({
  initialCaseStudyData,
  initialRelatedCaseStudies,
}: CaseStudyPostClientProps) {
  const [caseStudyData, setCaseStudyData] = useState(initialCaseStudyData);
  const [relatedCaseStudies, setRelatedCaseStudies] = useState(initialRelatedCaseStudies);
  ...
  const contentHtml = caseStudyData.contentHtml || '';
  ...
  return (
    <div className="min-h-screen bg-background text-foreground py-8 sm:py-12 md:py-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8"> {/* Wider container */}
        <Link href="/automations/case-studies" className="text-ph-accent hover:text-primary-foreground transition-colors duration-200 mb-6 inline-block text-sm sm:text-base">
          &larr; Back to Case Studies
        </Link>

        {/* Article Header */}
        <header className="mb-8 md:mb-12">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4 leading-tight">{caseStudyData.title || 'Untitled Case Study'}</h1>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center text-ph-light-gray text-sm sm:text-base mb-4 space-y-1 sm:space-y-0 sm:space-x-4">
            <span>By <span className="font-semibold text-white">{caseStudyData.author || 'Admin'}</span></span>
            <span className="hidden sm:inline">•</span>
            <span>{format(caseStudyData.publicationDate, 'MMMM dd, yyyy')}</span>
            <span className="hidden sm:inline">•</span>
            <span>{readTime}</span>
          </div>
          ...
          {caseStudyData.tags && caseStudyData.tags.length > 0 && (
            <div className="mb-6 flex flex-wrap gap-2">
              {caseStudyData.tags.map((tag: string) => (
                <span
                  key={tag}
            ...
            {showShareOptions && (
              <div className="origin-top-right absolute left-0 mt-2 w-56 rounded-md shadow-lg bg-card ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                  ...
                  <a href={`https://twitter.com/intent/tweet?url=${currentUrl}&text=${encodeURIComponent(caseStudyData.title || '')}`} target="_blank" rel="noopenernoreferrer" className="block px-4 py-2 text-sm text-ph-light-gray hover:bg-muted hover:text-white" role="menuitem">
                    Share on Twitter
                  </a>
                  <a href={`https://www.linkedin.com/shareArticle?mini=true&url=${currentUrl}&title=${encodeURIComponent(caseStudyData.title || '')}`} target= "_blank" rel="noopener noreferrer" className="block px-4 py-2 text-sm text-ph-light-gray hover:bg-muted hover:text-white" role="menuitem"> 
                    Share on LinkedIn
                  </a>
        ...
        {/* Featured Image */}
        {caseStudyData.featuredImage?.url && (
          <div className="mb-8">
            <div className="relative w-full h-72 sm:h-96 md:h-[500px] lg:h-[600px] rounded-lg overflow-hidden"> {/* More prominent image */}
              <Image
                src={caseStudyData.featuredImage.url}
                alt={caseStudyData.featuredImage.title || caseStudyData.title || 'Image'}
                title={caseStudyData.featuredImage.description || caseStudyData.title || 'Image'}
                fill
                style={{ objectFit: 'cover' }}
                className="rounded-lg"
              />
            </div>
            {caseStudyData.featuredImage?.description && (
              <p className="text-center text-sm text-ph-light-gray mt-2">{caseStudyData.featuredImage.description}</p>
            )}
          </div>
        )}
