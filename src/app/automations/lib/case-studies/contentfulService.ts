...
export const getContentfulCaseStudies = async (): Promise<ContentfulCaseStudy[]> => {
  try {
...
export const getContentfulCaseStudyBySlug = async (slug: string): Promise<ContentfulCaseStudy | null> => {
  try {
    const entries: EntryCollection<any> = await contentfulClient.getEntries({
      content_type: 'caseStudies',
      'fields.slug': slug,
